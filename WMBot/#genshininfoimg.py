...
def char_id_to_name(udata: ysstructs.GenshinUserData, charid: int):
    chars = udata.avatars
    for char in chars:
        if charid == char.id:
            return char.name
...
def abyssAnalysis(aby: ysstructs.GenshinAbyss, udata: ysstructs.GenshinUserData):
    ...
    if len(aby['floors']) >= 0:
        if len(aby['floors'][0]['levels']) > 0:
            has_details = True
            for floor in aby['floors']: 
                room_text = ""  
                for room in floor['levels']:  
                    battle_text = ""  
                    for battle in room['battles']:  
                        character_text = ""  
                        for char in battle['avatars']:  
